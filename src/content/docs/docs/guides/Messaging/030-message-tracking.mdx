...
This handler will automatically join the **default command consumer**. If a consumer is configured through `FluxzeroBuilder` and matches the handler, that configuration is used before the app default. An explicit `@Consumer` on the handler remains most specific and therefore takes precedence.
