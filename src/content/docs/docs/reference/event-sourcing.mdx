...
SmartHome home1 = FluxCapacitor.loadAggregate(homeId, SmartHome.class);
// Must replay all 10,000 events

// With snapshot: O(k) where k = events since last snapshot  
@Aggregate(snapshotPeriod = 1000)
SmartHome home2 = FluxCapacitor.loadAggregate(homeId, SmartHome.class);
...
DeviceCatalog catalog = FluxCapacitor.loadAggregate(catalogId, DeviceCatalog.class);  
