...
Documents can be automatically indexed when marked as `@Searchable`, or manually indexed using the FluxCapacitor API.
</Card>
</CardGrid>
...
You can manually index documents using the FluxCapacitor API:
...
public class DeviceCatalogService {
    ...
    public void addDeviceTocatalog(DeviceCatalogEntry device) {
        FluxCapacitor.index(device, device.catalogId(), "device-catalog");
    }
    
    // Index with conditional logic
    public void addDeviceIfNew(DeviceCatalogEntry device) {
        CompletableFuture<Void> future = FluxCapacitor
...
public class ReviewService {
    ...
    public void addReview(DeviceReview review) {
        ...
        // No explicit FluxCapacitor.index() call needed
...
public class DeviceSearchService {
    ...
    public List<DeviceCatalogEntry> searchWithFilters(
        ...
        var search = FluxCapacitor.search(DeviceCatalogEntry.class);
...
class DeviceSearchService {
    ...
    fun searchWithFilters(
        ...
        var search = FluxCapacitor.search(DeviceCatalogEntry::class.java)
...
List<DeviceCatalogEntry> devices = FluxCapacitor
    .search(DeviceCatalogEntry.class)
...
// Slower:
List<DeviceCatalogEntry> slow = FluxCapacitor
    .search(DeviceCatalogEntry.class)
