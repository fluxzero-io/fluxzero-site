...
Audit trail and Logs support search and histogram range selection. Issues starts on All and supports individual and bulk local status changes, application/severity filters, sortable counts and occurrence details. Its histogram supports time selection and a native zoom-out control; counts and visible issues follow the selected range. Open still removes an issue from view when it is resolved. Documents offers Reservation, Ticket and Show, with simple text search and native document details. Trace and Insights retain their real graph interactions. The parent page keeps semantic descriptions  for accessibility and Markdown/llms exports. Frames remain keyboard-accessible.
...
Desktop wheel gestures that start on the website continue scrolling the page when they cross a frame. After a short pause, a fresh gesture inside an example can scroll its table normally, without clicking to activate it. Same-origin messages share page gesture activity and are checked against the parent and its own frames. Native inspect hints remain visible. The audit/log range control exposes only the native zoom-out icon after a histogram selection.
