...
SmartHome home = Fluxzero.loadAggregateFor(roomId).get();

// Load just the room entity
Room room = Fluxzero.loadEntity(roomId).get();

// Load just the device entity
Device device = Fluxzero.loadEntity(deviceId).get();
```
</TabItem>
<TabItem label="Kotlin">
```kotlin
// Load aggregate containing a specific room
val home = Fluxzero.loadAggregateFor(roomId).get()

// Load just the room entity
val room = Fluxzero.loadEntity(roomId).get()

// Load just the device entity
val device = Fluxzero.loadEntity(deviceId).get()
```
</TabItem>
</Tabs>
