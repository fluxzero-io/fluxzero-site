...
public class DeviceCommandHandler {
    ...
    public DeviceId handle(AddDevice command) {
        ...
        // Add device to home
        SmartHome home = Fluxzero.loadAggregate(command.homeId(), SmartHome.class);
        Device newDevice = command.device().toBuilder()
            .deviceId(DeviceId.generate())
            .build();
            
        Fluxzero.sendCommand(new UpdateSmartHome(
            command.homeId(),
            home.toBuilder()
        ...
        // Publish event
        Fluxzero.publishEvent(new DeviceAdded(
            command.homeId(), 
            newDevice, 
            Instant.now()
        ));
    ...
    public void handle(TurnOnLight command) {
        Device light = Fluxzero.loadEntity(command.lightId());
        ...
        Fluxzero.updateEntity(updatedLight);
        
        // Publish event
        Fluxzero.publishEvent(new LightTurnedOn(
            command.lightId(),
    ...
    public void handle(SetThermostatTemperature command) {
        Device thermostat = Fluxzero.loadEntity(command.thermostatId());
        ...
        Fluxzero.updateEntity(updatedThermostat);
        
        // Publish event
        Fluxzero.publishEvent(new ThermostatTemperatureChanged(
            command.thermostatId(),
            oldTemperature,
            command.temperature(),
            "manual"
...
class DeviceCommandHandler {
    ...
    fun handle(command: AddDevice): DeviceId {
        ...
        // Add device to home
        val home = Fluxzero.loadAggregate(command.homeId, SmartHome::class.java)
        val newDevice = command.device.copy(deviceId = DeviceId.generate())
        
        Fluxzero.sendCommand(UpdateSmartHome(
            command.homeId,
            home.copy(devices = home.devices + newDevice)
        ))
        
        // Publish event
        Fluxzero.publishEvent(DeviceAdded(
            command.homeId, 
            newDevice, 
            Instant.now()
        ))
    ...
    fun handle(command: TurnOnLight) {
        val light = Fluxzero.loadEntity<Device>(command.lightId)
        ...
        // Publish event
        Fluxzero.publishEvent(LightTurnedOn(
            command.lightId,
            getRoomForDevice(command.lightId),
            command.brightness
        ))
    ...
    fun handle(command: SetThermostatTemperature) {
        val thermostat = Fluxzero.loadEntity<Device>(command.thermostatId)
        ...
        // Publish event
        Fluxzero.publishEvent(ThermostatTemperatureChanged(
            command.thermostatId,
            oldTemperature,
            command.temperature,
            "manual"
...
public class SecurityEventHandler {
    ...
    public void on(MotionDetected event) {
        ...
        if (home.isSecurityArmed()) {
            ...
            Fluxzero.publishEvent(new SecurityBreach(
                home.homeId(),
                event.sensorId(),
                "motion_while_armed",
                "HIGH"
            ...
            Fluxzero.sendCommand(new SendNotification(
                home.homeId(),
    ...
    public void on(DeviceOffline event) {
        Device device = Fluxzero.loadEntity(event.deviceId());
        
        // Check if it's a critical security device
        if (isSecurityDevice(device)) {
            SmartHome home = Fluxzero.loadAggregateFor(event.deviceId());
            
            Fluxzero.publishEvent(new SecurityDeviceOffline(
                home.homeId(),
                event.deviceId(),
                device.type(),
            ...
            if (event.offlineDuration().toHours() > 24) {
                Fluxzero.sendCommand(new ScheduleMaintenance(
                    home.homeId(),
                    event.deviceId(),
    ...
    public void on(DoorUnlocked event) {
        ...
        if (isRestrictedHours()) {
            Fluxzero.publishEvent(new UnauthorizedAccess(
...
public class AutomationEventHandler {
    ...
    public void on(MotionDetected event) {
        ...
        for (DeviceId lightId : roomLights) {
            Device light = Fluxzero.loadEntity(lightId);
            
            if (light.isOnline() && !isLightOn(light)) {
                int brightness = calculateOptimalBrightness(roomId);
                
                Fluxzero.sendCommand(new TurnOnLight(lightId, brightness));
            }
        }
        ...
        for (AutomationRule rule : motionRules) {
            if (rule.isEnabled()) {
                Fluxzero.sendCommand(new ExecuteAutomationRule(
                    rule.ruleId(),
    ...
    public void on(TemperatureReading event) {
        ...
        for (DeviceId thermostatId : thermostats) {
            Device thermostat = Fluxzero.loadEntity(thermostatId);
            ...
            if (thermostat.isOnline()) {
                ...
                if (Math.abs(event.temperature() - targetTemp) > 2.0) {
                    Fluxzero.sendCommand(new AutoAdjustThermostat(
                        thermostatId,
                        event.temperature(),
                        targetTemp
                    ));
...
class SecurityEventHandler {
    ...
    fun on(event: MotionDetected) {
        ...
        if (home.isSecurityArmed) {
            ...
            // Trigger security breach
            Fluxzero.publishEvent(SecurityBreach(
                home.homeId,
                event.sensorId,
                "motion_while_armed",
                "HIGH"
            ...
            // Send immediate notification
            Fluxzero.sendCommand(SendNotification(
                home.homeId,
                "Security Alert: Motion detected while system armed",
                NotificationPriority.URGENT
            ))
    ...
    fun on(event: DeviceOffline) {
        val device = Fluxzero.loadEntity<Device>(event.deviceId)
        
        // Check if it's a critical security device
        if (isSecurityDevice(device)) {
            val home = Fluxzero.loadAggregateFor(event.deviceId)
            
            Fluxzero.publishEvent(SecurityDeviceOffline(
                home.homeId,
                event.deviceId,
                device.type,
                event.offlineDuration
            ...
            // If offline too long, trigger maintenance alert
            if (event.offlineDuration.toHours() > 24) {
                Fluxzero.sendCommand(ScheduleMaintenance(
                    home.homeId,
                    event.deviceId,
                    "Device offline for extended period"
                ))
    ...
    fun on(event: DoorUnlocked) {
        ...
        // Check if access is during restricted hours
        if (isRestrictedHours()) {
            Fluxzero.publishEvent(UnauthorizedAccess(
                getHomeId(event.lockId),
                event.lockId,
                "Access during restricted hours: ${event.unlockMethod}",
                getCurrentIpAddress()
...
class AutomationEventHandler {
    ...
    fun on(event: MotionDetected) {
        ...
        roomLights.forEach { lightId ->
            val light = Fluxzero.loadEntity<Device>(lightId)
            
            if (light.isOnline && !isLightOn(light)) {
                val brightness = calculateOptimalBrightness(roomId)
                
                Fluxzero.sendCommand(TurnOnLight(lightId, brightness))
            }
        }
        ...
        motionRules.filter { it.isEnabled }.forEach { rule ->
            Fluxzero.sendCommand(ExecuteAutomationRule(
                rule.ruleId,
    ...
    fun on(event: TemperatureReading) {
        ...
        thermostats.forEach { thermostatId ->
            val thermostat = Fluxzero.loadEntity<Device>(thermostatId)
            ...
            if (thermostat.isOnline) {
                ...
                if (kotlin.math.abs(event.temperature - targetTemp) > 2.0) {
                    Fluxzero.sendCommand(AutoAdjustThermostat(
                        thermostatId,
                        event.temperature,
                        targetTemp
                    ))
...
public class SmartHomeQueryHandler {
    ...
    public SmartHome handle(GetSmartHome query) {
        return Fluxzero.loadAggregate(query.homeId(), SmartHome.class);
    }
    
    @HandleQuery
    public List<Device> handle(GetDevicesByRoom query) {
        SmartHome home = Fluxzero.loadAggregateFor(query.roomId());
    ...
    public List<Device> handle(GetOnlineDevices query) {
        SmartHome home = Fluxzero.loadAggregate(query.homeId(), SmartHome.class);
    ...
    public DeviceStatus handle(GetDeviceStatus query) {
        Device device = Fluxzero.loadEntity(query.deviceId());
...
class SmartHomeQueryHandler {
    ...
    fun handle(query: GetSmartHome): SmartHome {
        return Fluxzero.loadAggregate(query.homeId, SmartHome::class.java)
    }
    
    @HandleQuery
    fun handle(query: GetDevicesByRoom): List<Device> {
        val home = Fluxzero.loadAggregateFor(query.roomId)
    ...
    fun handle(query: GetOnlineDevices): List<Device> {
        val home = Fluxzero.loadAggregate(query.homeId, SmartHome::class.java)
        return home.devices.filter { it.isOnline }
    }
    
    @HandleQuery
    fun handle(query: GetDeviceStatus): DeviceStatus {
        val device = Fluxzero.loadEntity<Device>(query.deviceId)
...
public class SmartHomeErrorHandler {
    ...
    public void handle(IllegalCommandException error, Object originalMessage) {
        ...
        if (originalMessage instanceof TurnOnLight command) {
            ...
            Fluxzero.sendCommand(new ScheduleDeviceUpdate(command.lightId()));
        }
    }
    ...
    public void handle(SecurityException error, Object command) {
        ...
        Fluxzero.sendCommand(new SendAdminAlert(
...
class SmartHomeErrorHandler {
    ...
    fun handle(error: IllegalCommandException, originalMessage: Any) {
        ...
        when (originalMessage) {
            is TurnOnLight -> {
                // Try fallback - maybe device needs firmware update
                Fluxzero.sendCommand(ScheduleDeviceUpdate(originalMessage.lightId))
            }
        }
    }
    ...
    fun handle(error: SecurityException, command: Any) {
        ...
        // Alert administrators
        Fluxzero.sendCommand(SendAdminAlert(
...
public class AdvancedHandlerExamples {
    ...
    public void on(MotionDetected event,
        ...
        roomDevices.stream()
            ...
            .forEach(light -> {
                Fluxzero.sendCommand(new TurnOnLight(
                    light.deviceId(),
...
class AdvancedHandlerExamples {
    ...
    fun on(event: MotionDetected,
        ...
        roomDevices
            ...
            .filter { !isLightOn(it) }
            .forEach { light ->
                Fluxzero.sendCommand(TurnOnLight(
                    light.deviceId,
                    calculateBrightness(room, event.detectedAt)
                ))
            }
...
public class RemoteDeviceHandler {
    ...
    public void handle(TurnOnLight command) {
        ...
        // Update device state
        Device device = Fluxzero.loadEntity(command.lightId());
        ...
        Fluxzero.updateEntity(updatedDevice);
...
class RemoteDeviceHandler {
    ...
    fun handle(command: TurnOnLight) {
        ...
        // Update device state
        val device = Fluxzero.loadEntity<Device>(command.lightId)
