...
You are allowed to use the `$` prefix for your own metadata properties, but this is unsupported and therefore we
recommend using a different prefix to avoid conflicts with Fluxzero's internal metadata properties.
Supported exception: use the SDK-provided trace helpers (for example `Metadata.withTrace(...)`) which intentionally
write `$trace.*` keys.
</Aside>
