...
Create a new project called 'Todo' using the Basic Starter for Java or Kotlin (see [Installation](/docs/getting-started/installation) for how).
...
<Aside type="note">
  Behind the scenes, messages like commands are consumed from the command log in Fluxzero by your app’s
  default command consumer.

  If you want more control, you can also create an isolated consumer with <code>@Consumer</code>.
  For details on consumer behavior, tuning, and tracking positions, see
  [Message tracking](/docs/guides/messaging/030-message-tracking/).
</Aside>
...
Want to understand why we recommend reusing the same payload in both command and event? See [Applying entity updates](/docs/guides/modeling--persistence/170-domain-modeling#applying-updates-to-entities).
...
Fluxzero Search makes queries like this effortless. See the [search docs](/docs/guides/modeling--persistence/220-document-index-and-search/) for details.
...
For more details, see the [handling web requests guide](/docs/guides/messaging/120-handling-web-requests/).
...
<Aside type="note">
  ...
  See [User providers](/docs/guides/messaging/071-role-based-access-control/#where-does-user-info-come-from) for more info.
</Aside>
...
<Aside type="note">
  ...
  was when the message occurred. See [Entity loading](/docs/guides/modeling--persistence/180-entity-loading/) for more info.
</Aside>
...
<Aside type="note">
  ...
  For more information on injecting application properties, [see here](/docs/guides/configuration--support/280-application-properties/).
</Aside>
...
- Dive deeper with our [developer guides](/docs/guides/010-overview)
- Learn how to [deploy to Fluxzero Cloud](/docs/tutorials/cloud-deployment)
