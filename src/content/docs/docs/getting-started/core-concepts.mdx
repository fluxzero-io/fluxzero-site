...
<Aside type="note">
    ...
    In previous examples, such as with queries, we didn't need to register anything manually because the query class itself contained the handler, allowing Fluxzero to [automatically discover it](/docs/guides/configuration-and-support/configuring-fluxzero/#spring-auto-configuration).
