...
### 1. Message Dispatch
When you call `Fluxzero.sendCommand(new CreateUser("Alice"))`, your message is:
...
```java
Fluxzero.sendCommand(new CreateUser("Alice"));
```
...
```java
Fluxzero.publishEvent(new UserCreated("Alice", userId));
```
...
```java
User user = Fluxzero.queryAndWait(new GetUser(userId));
```
...
3. **Send messages** - Use `Fluxzero.sendCommand()`, `publishEvent()`, etc.
