...
public class GetRoomTemperature {
    ...
    TemperatureReading handle() {
        ...
        Room room = Fluxzero.loadEntity(roomId);
...
data class GetRoomTemperature(val roomId: RoomId) {
    ...
    fun handle(): TemperatureReading {
        // Load the room and get current temperature
        val room = Fluxzero.loadEntity<Room>(roomId)
...
public class SmartHomeService {
    ...
    public DeviceStatus checkDevice(DeviceId deviceId) {
        ...
        return Fluxzero.queryAndWait(new GetDeviceStatus(deviceId));
    }
    
    public SecurityStatus getSecurityStatus(HomeId homeId) {
        // Compile-time type checking ensures correct return type
        return Fluxzero.queryAndWait(new GetHomeSecurityStatus(homeId));
    }
...
class SmartHomeService {
    ...
    fun checkDevice(deviceId: DeviceId): DeviceStatus {
        // Type-safe query - returns DeviceStatus
        return Fluxzero.queryAndWait(GetDeviceStatus(deviceId))
    }
    
    fun getSecurityStatus(homeId: HomeId): SecurityStatus {
        // Compile-time type checking ensures correct return type
        return Fluxzero.queryAndWait(GetHomeSecurityStatus(homeId))
    }
...
public class SmartHomeDashboardService {
    ...
    public DashboardData loadDashboard(HomeId homeId) {
        ...
        SecurityStatus security = Fluxzero.queryAndWait(
            new GetHomeSecurityStatus(homeId));
        
        // Get all online devices
        List<Device> onlineDevices = Fluxzero.queryAndWait(
            new GetOnlineDevices(homeId));
        
        // Get today's energy usage
        EnergyUsageSummary energyUsage = Fluxzero.queryAndWait(
            new GetEnergyUsageSummary(homeId, LocalDate.now()));
        
        // Get recent alerts
        List<SecurityAlert> alerts = Fluxzero.queryAndWait(
...
class SmartHomeDashboardService {
    ...
    fun loadDashboard(homeId: HomeId): DashboardData {
        ...
        // Get current security status
        val security = Fluxzero.queryAndWait(
            GetHomeSecurityStatus(homeId))
        
        // Get all online devices
        val onlineDevices = Fluxzero.queryAndWait(
            GetOnlineDevices(homeId))
        
        // Get today's energy usage
        val energyUsage = Fluxzero.queryAndWait(
            GetEnergyUsageSummary(homeId, LocalDate.now()))
        
        // Get recent alerts
        val alerts = Fluxzero.queryAndWait(
...
public class AsyncSmartHomeService {
    ...
    public CompletableFuture<DashboardData> loadDashboardAsync(HomeId homeId) {
        ...
        CompletableFuture<SecurityStatus> securityFuture = 
            Fluxzero.query(new GetHomeSecurityStatus(homeId));
            
        CompletableFuture<List<Device>> devicesFuture = 
            Fluxzero.query(new GetOnlineDevices(homeId));
            
        CompletableFuture<EnergyUsageSummary> energyFuture = 
            Fluxzero.query(new GetEnergyUsageSummary(homeId, LocalDate.now()));
            
        CompletableFuture<List<SecurityAlert>> alertsFuture = 
            Fluxzero.query(new GetRecentSecurityAlerts(homeId, Duration.ofHours(24)));
    ...
    public CompletableFuture<DeviceControlResponse> controlDeviceAsync(DeviceId deviceId, boolean turnOn) {
        ...
        return Fluxzero.query(new GetDeviceStatus(deviceId))
            .thenCompose(status -> {
                if (status.isOnline()) {
                    if (turnOn) {
                        return Fluxzero.sendAndWait(new TurnOnDevice(deviceId))
                            .thenApply(result -> DeviceControlResponse.success());
                    } else {
                        return Fluxzero.sendAndWait(new TurnOffDevice(deviceId))
                            .thenApply(result -> DeviceControlResponse.success());
                    }
                } else {
...
class AsyncSmartHomeService {
    ...
    fun loadDashboardAsync(homeId: HomeId): CompletableFuture<DashboardData> {
        ...
        val securityFuture = Fluxzero.query(GetHomeSecurityStatus(homeId))
        val devicesFuture = Fluxzero.query(GetOnlineDevices(homeId))
        val energyFuture = Fluxzero.query(GetEnergyUsageSummary(homeId, LocalDate.now()))
        val alertsFuture = Fluxzero.query(GetRecentSecurityAlerts(homeId, Duration.ofHours(24)))
    ...
    fun controlDeviceAsync(deviceId: DeviceId, turnOn: Boolean): CompletableFuture<DeviceControlResponse> {
        // First query current state, then issue command
        return Fluxzero.query(GetDeviceStatus(deviceId))
            .thenCompose { status ->
                if (status.isOnline) {
                    if (turnOn) {
                        Fluxzero.sendAndWait(TurnOnDevice(deviceId))
                            .thenApply { DeviceControlResponse.success() }
                    } else {
                        Fluxzero.sendAndWait(TurnOffDevice(deviceId))
                            .thenApply { DeviceControlResponse.success() }
                    }
                } else {
...
public class ReportService {
    ...
    public CompletableFuture<EnergyReport> generateReportWithTimeout(HomeId homeId) {
        return Fluxzero.query(new GenerateEnergyReport(homeId,
...
class ReportService {
    ...
    fun generateReportWithTimeout(homeId: HomeId): CompletableFuture<EnergyReport> {
        return Fluxzero.query(GenerateEnergyReport(homeId,
                LocalDate.now().minusDays(30), LocalDate.now()))
            .orTimeout(10, TimeUnit.SECONDS)
    }
}
...
public class SmartHomeWebController {
    ...
    CompletableFuture<DashboardData> getDashboard(@PathVariable HomeId homeId) {
        ...
        return Fluxzero.query(new GetDashboardData(homeId));
    }
    ...
    CompletableFuture<List<Device>> searchDevices(@QueryParam String term,
        ...
        return Fluxzero.query(new SearchDevices(homeId, term, type));
    }
    ...
    EnergyUsageSummary getEnergyUsage(@PathVariable HomeId homeId,
        ...
        return Fluxzero.queryAndWait(new GetEnergyUsageSummary(homeId, queryDate));
    }
}
```
</TabItem>
...
class SmartHomeWebController {
    ...
    fun getDashboard(@PathVariable homeId: HomeId): CompletableFuture<DashboardData> {
        // Web endpoint directly handles query
        return Fluxzero.query(GetDashboardData(homeId))
    }
    ...
    fun searchDevices(@QueryParam term: String,
        ...
        return Fluxzero.query(SearchDevices(homeId, term, type))
    }
    ...
    fun getEnergyUsage(@PathVariable homeId: HomeId,
        ...
        return Fluxzero.queryAndWait(GetEnergyUsageSummary(homeId, queryDate))
    }
}
```
</TabItem>
...
DeviceStatus status = Fluxzero.queryAndWait(new GetDeviceStatus(deviceId));
...
List<Device> devices = Fluxzero.search(Device.class)
    .match("true", "isOnline")
...
public class SmartHomeDashboardService {
    ...
    public CompletableFuture<SmartHomeDashboard> loadDashboard(HomeId homeId) {
        ...
        CompletableFuture<SecurityStatus> securityFuture = 
            Fluxzero.query(new GetSecurityStatus(homeId));
            
        CompletableFuture<List<Device>> devicesFuture = 
            Fluxzero.query(new GetOnlineDevices(homeId));
            
        CompletableFuture<EnergyUsageSummary> energyFuture = 
            Fluxzero.query(new GetTodaysEnergyUsage(homeId));
            
        CompletableFuture<List<AutomationRule>> rulesFuture = 
            Fluxzero.query(new GetActiveAutomationRules(homeId));
            
        CompletableFuture<WeatherForecast> weatherFuture = 
            Fluxzero.query(new GetLocalWeatherForecast(homeId));
...
class SmartHomeDashboardService {
    ...
    fun loadDashboard(homeId: HomeId): CompletableFuture<SmartHomeDashboard> {
        // Parallel queries for dashboard data
        val securityFuture = Fluxzero.query(GetSecurityStatus(homeId))
        val devicesFuture = Fluxzero.query(GetOnlineDevices(homeId))
        val energyFuture = Fluxzero.query(GetTodaysEnergyUsage(homeId))
        val rulesFuture = Fluxzero.query(GetActiveAutomationRules(homeId))
        val weatherFuture = Fluxzero.query(GetLocalWeatherForecast(homeId))
