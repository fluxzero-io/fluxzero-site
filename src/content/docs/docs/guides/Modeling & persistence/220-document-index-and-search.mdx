...
Search is optimized for current state, but indexing still follows the commit path. When a command directly updates a
...
This does not cover downstream projections or side-effect indexing. If a document is indexed by an event handler or
projection handler, treat it as eventually consistent and wait for that projection's own completion signal when immediate
