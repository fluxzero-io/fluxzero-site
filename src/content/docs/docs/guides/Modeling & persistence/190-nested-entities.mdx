...
description: Modeling and updating nested entities within aggregates
sidebar:
   order: 190
---
...
Java records are rebuilt through their canonical constructor, so `@Member` record components do not need `@With`. Kotlin data classes are rebuilt through copy semantics. Custom immutable classes can still use explicit withers or builders.
</Aside>
...
### Strongly typed @Alias identifiers
